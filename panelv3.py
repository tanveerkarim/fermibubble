...
#Read FITS files
g130m = fits.open('../RBS1768/lc1201010_x1dsum.fits')
g160m = fits.open('../RBS1768/lc1201020_x1dsum.fits')

#Read GASS HI spectra file
hi = np.genfromtxt("../RBS1768/GASS_HI_spectrum_mod.txt")
hi = hi.T

#Extract Wavelength, Flux and Error
w130 = g130m[1].data['WAVELENGTH']
f130 = g130m[1].data['FLUX']
e130 = g130m[1].data['ERROR']

w160 = g160m[1].data['WAVELENGTH']
f160 = g160m[1].data['FLUX']
e160 = g160m[1].data['ERROR']

"""--------------------------------------------------------------------------------------------------------------------------------------------------"""

#Produces the entire spectra
#Combine both grating
w = np.concatenate((w130.ravel(),w160.ravel()))
f = np.concatenate((f130.ravel(),f160.ravel()))
e = np.concatenate((e130.ravel(),e160.ravel()))

#Combine w and f to form a 2-d array for the rebin function
a130 = np.array([w130.ravel(),f130.ravel()])
a160 = np.array([w160.ravel(),f160.ravel()])
a = np.array([w,f])

#Collapse by 4 "pixel"
tmp130 = bin_ndarray(a130,(2,(len(w130.ravel())/4)),'avg')
tmp160 = bin_ndarray(a160,(2,(len(w160.ravel())/4)),'avg')
tmp = bin_ndarray(a,(2,(len(w)/4)),'avg')
tmp_hi = bin_ndarray(hi,(2,(1748/4)),'avg')
...
axes[0][1].plot(tmp_hi[0],tmp_hi[1],'k')
...
axes[4][1].text(-380,.2,'Si IV 1393',fontsize=15,fontname="serif")
